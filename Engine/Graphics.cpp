...
void Graphics::DrawSpriteColor(int x, int y, const Surface& s, Color spriteColor, Color chroma)
{
	DrawSpriteColor(x, y, s.GetRect(), s, spriteColor, chroma);
}

void Graphics::DrawSpriteColor(int x, int y, const RectI& srcRect, const Surface& s, Color spriteColor, Color chroma)
{
	DrawSpriteColor(x, y, srcRect, GetScreenRect(), s, spriteColor, chroma);
}

void Graphics::DrawSpriteColor(int x, int y, RectI srcRect, const RectI& clip, const Surface& s, Color spriteColor, Color chroma)
{
	assert(srcRect.left >= 0);
	assert(srcRect.right <= s.GetWidth());
	assert(srcRect.top >= 0);
	...
	for (int sy = srcRect.top; sy < srcRect.bottom; sy++)
	{
		for (int sx = srcRect.left; sx < srcRect.right; sx++)
		{
			...
			if (srcPixel != chroma)
			{
				PutPixel(x + sx - srcRect.left, y + sy - srcRect.top, spriteColor);
			}
		}
	}
}
